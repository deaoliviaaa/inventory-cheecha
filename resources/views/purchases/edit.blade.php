<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Edit Purchase') }}</h2>
    </x-slot>
            ...
            <div class="bg-white shadow-sm sm:rounded-lg p-6 border border-gray-200">
                <form action="{{ route('purchases.update', $purchase) }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="kode_pembelian" value="{{ old('kode_pembelian', $purchase->kode_pembelian) }}" class="w-full border rounded px-3 py-2">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" name="tanggal_pembelian" value="{{ old('tanggal_pembelian', $purchase->tanggal_pembelian->format('Y-m-d')) }}" class="w-full border rounded px-3 py-2">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="nama_barang" value="{{ old('nama_barang', $purchase->nama_barang) }}" class="w-full border rounded px-3 py-2">
                        @error('nama_barang') <div class="text-red-600 text-sm">{{ $message }}</div> @enderror
                    </div>

                    <div class="mb-4">
                        <label class="block font-medium mb-1">Quantity</label>
                        <input type="number" name="jumlah_stok" value="{{ old('jumlah_stok', $purchase->jumlah_stok) }}" class="w-full border rounded px-3 py-2" min="1">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="number" name="harga_total" value="{{ old('harga_total', $purchase->harga_total) }}" class="w-full border rounded px-3 py-2" step="0.01">
                        @error('harga_total') <div class="text-red-600 text-sm">{{ $message }}</div> @enderror
                    </div>

                    <div class="flex gap-2">
                        <button type="submit" class="px-4 py-2 bg-cheecha text-white rounded hover:bg-cheecha-dark">Update</button>
